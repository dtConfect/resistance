...
class ScepticBot(Bot):
	...
	def onGameRevealed(self, players, spies):
		...
		self.otherPlayers = [p for p in players if p.index != self.index]
	...
	def calculateCurrentPlayerSuspicions(self):
		...
		for p in [ p for p in self.playerInfos if p.player.index != self.index ]:
			currentSuspicions[p.player.name] = p.suspicion
	...
	def onGameComplete(self, win, spies):
		...
		#print("Sceptic suspicion statistics: ")
